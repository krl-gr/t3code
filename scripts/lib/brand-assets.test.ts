...
describe("brand-assets", () => {
  ...
  it("keeps development, nightly, and production icon families separate", () => {
    ...
    // Nightly and production point at the Up.computer artwork, not upstream's
    // `nightly-*` / `black-*` families, which stay in the tree unused.
    expect(BRAND_ASSET_PATHS.developmentDesktopIconPng).toMatch(/^assets\/dev\/blueprint-/);
    expect(BRAND_ASSET_PATHS.nightlyMacIconPng).toMatch(/^assets\/nightly\/blueprint-/);
