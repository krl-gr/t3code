...
- Publishes the CLI package (`apps/server`, npm package `@updotcomputer/cli`) to the `nightly` npm dist-tag using the same nightly version.
...
1. Confirm npm org/user owns package `@updotcomputer/cli`.
