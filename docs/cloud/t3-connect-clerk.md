...
Local desktop development uses `upcomputer-dev://app`, while packaged builds use `upcomputer://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `upcomputer-dev://app`; the production Clerk instance should only need
`upcomputer://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["upcomputer://app"]}'
```
...
The production macOS bundle ID is `computer.up.upcomputer`. To enable native passkeys:

1. Create an explicit macOS App ID for `computer.up.upcomputer` in the Apple Developer portal and enable
   **Associated Domains**.
...
```sh
codesign --verify --deep --strict "/Applications/Up.computer (Alpha).app"
codesign -d --entitlements :- "/Applications/Up.computer (Alpha).app"
```
