...
This guide is for people who want to use more than one Codex account in Up.computer.
...
- both accounts can see the same Codex sessions
...
In Up.computer Settings, name it something obvious:
...
In Up.computer Settings, add another Codex provider:
...
Up.computer shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Up.computer treats it
