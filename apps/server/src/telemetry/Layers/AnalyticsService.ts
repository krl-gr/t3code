...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("T3CODE_POSTHOG_KEY").pipe(Config.withDefault("phc_uqRUAQavAKuUny7uFxm8tqr6nyVDpdoSARKsWNzs8wBx")),
...
const makeAnalyticsService = Effect.gen(function* () {
  ...
  if (!telemetryConfig.enabled || posthogKey.length === 0) {
    const record: AnalyticsServiceShape["record"] = Effect.fn("record")(
      function* (_event, _properties) {},
    );
