...
describe("threadContext", () => {
  it("snapshot captures completed user and assistant messages only", () => {
    ...
    assert.notStrictEqual(typeof binding, "string");
    if (typeof binding === "string") return;
    assert.match(binding.snapshotText ?? "", /USER:\ncompleted user/);
    assert.match(binding.snapshotText ?? "", /ASSISTANT:\ncompleted assistant/);
    assert.doesNotMatch(binding.snapshotText ?? "", /streaming assistant/);
  });
  ...
  it("snapshot cutoff must point at a completed assistant message", () => {
    ...
    assert.notStrictEqual(typeof accepted, "string");
    if (typeof accepted === "string") return;
    assert.match(accepted.snapshotText ?? "", /ASSISTANT:\nanswer/);
    assert.doesNotMatch(accepted.snapshotText ?? "", /next ask/);
  });
  ...
  it("rejects snapshot bindings when the source has no completed context messages", () => {
    ...
    assert.equal(rejected, "Source thread has no completed messages to attach.");
  });
  ...
  it("materializes stored snapshot context without needing source messages", () => {
    ...
    assert.equal(materialized.blocks.length, 1);
    assert.match(materialized.blocks[0]?.text ?? "", /USER:\nfirst ask/);
    assert.match(materialized.blocks[0]?.text ?? "", /ASSISTANT:\nfirst answer/);
  });
  ...
  it("does not leak full snapshot text when the remaining context budget is tiny", () => {
    ...
    assert.equal(materialized.blocks.length, 1);
    assert.equal(materialized.blocks[0]?.text.length, 4);
    assert.doesNotMatch(materialized.blocks[0]?.text ?? "", /secret transcript/);
  });
  ...
  it("rejects self-binding and cycles", () => {
    ...
    assert.notStrictEqual(typeof bBinding, "string");
    ...
    assert.notStrictEqual(typeof cBinding, "string");
