// This file mostly exists because we want dev mode to say "Up.computer (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Up.computer (Dev)" : "Up.computer (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `computer.up.upcomputer.dev.${devBundleIdSuffix || "local"}`
  : "computer.up.upcomputer";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["upcomputer-dev"] : ["upcomputer"];
// Tracks upstream's counter so a launcher that already extracted at their
// version still re-extracts after this sync.
