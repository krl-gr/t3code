...
const createDesktopBridgeStub = (overrides?: {
  ...
  return {
    ...
    getThreadPromptDrafts: vi.fn().mockResolvedValue([]),
    setThreadPromptDrafts: vi.fn().mockResolvedValue(undefined),
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(
        page.getByText("Up.computer will restart to expose this environment over the network."),
      )
      .toBeInTheDocument();
