...
export function AttachChatContextPicker({
  ...
  useEffect(() => {
    if (!highlightedSourceId || !listRef.current) {
      return;
    }
    const element = listRef.current.querySelector<HTMLElement>(
      `[data-thread-id="${CSS.escape(highlightedSourceId)}"]`,
    );
    element?.scrollIntoView({ block: "nearest" });
  }, [highlightedSourceId]);

  const selectHighlightedSource = () => {
    if (!highlightedSourceId) return;
    void onSelectSource(highlightedSourceId);
  };
    ...
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="border-border/60 border-b px-3.5 py-3">
          <div className="text-sm font-medium">Attach context</div>
        </div>
