...
vi.mock("@t3tools/client-runtime", async (importOriginal) => {
  ...
  const stubWsClient: WsRpcClient = {
    ...
    server: {
      ...
      getBrowserProfileSnapshot: vi.fn(),
      openBrowserLoginWindow: vi.fn(),
      closeBrowserProfile: vi.fn(),
      clearBrowserProfile: vi.fn(),
      getComputerUseSnapshot: vi.fn(),
      restartComputerUse: vi.fn(),
      stopComputerUse: vi.fn(),
      refreshComputerUseTools: vi.fn(),
