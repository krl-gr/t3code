...
export function makeWsRpcProtocolClientForGroup<Rpcs extends Rpc.Any>(
  group: RpcGroup.RpcGroup<Rpcs>,
) {
  return RpcClient.make(group);
}

export const makeWsRpcProtocolClient = makeWsRpcProtocolClientForGroup(WsRpcGroup);
  ...
  const trackingWebSocketConstructorLayer = Layer.succeed(
    ...
    (socketUrl, protocols) => {
      ...
      socket.addEventListener(
        "error",
        () => {
          lifecycle.onError("Unable to connect to the Up.computer server WebSocket.");
        },
        { once: true },
      );
      socket.addEventListener(
