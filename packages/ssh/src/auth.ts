...
# Invoked by ssh via SSH_ASKPASS when ${PRODUCT_BASE_NAME} re-runs ssh with a cached password
...
printf '${PRODUCT_BASE_NAME} ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when ${PRODUCT_BASE_NAME} re-runs\r
...
[Console]::Error.WriteLine("${PRODUCT_BASE_NAME} ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
