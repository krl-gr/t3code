...
const PUBLISHABLE_UPCOMPUTER_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
const REMOTE_UPCOMPUTER_CLI_PACKAGE_NAME = "@updotcomputer/cli";
...
export function resolveRemoteUpcomputerCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_UPCOMPUTER_VERSION_PATTERN.test(appVersion)) {
    return `${REMOTE_UPCOMPUTER_CLI_PACKAGE_NAME}@${appVersion}`;
  }

  if (input.isDevelopment) {
    return `${REMOTE_UPCOMPUTER_CLI_PACKAGE_NAME}@nightly`;
  }

  return input.updateChannel === "nightly"
    ? `${REMOTE_UPCOMPUTER_CLI_PACKAGE_NAME}@nightly`
    : `${REMOTE_UPCOMPUTER_CLI_PACKAGE_NAME}@latest`;
}
